...
from .constants import CHECKPOINT_PREFIX
from .models import (
...
from .session import BedrockAgentRuntimeSessionClient
from .utils import (
...
class BedrockSessionSaver(BaseCheckpointSaver):
    ...
    def _create_session_invocation(self, thread_id: str, invocation_id: str):
    ...
    def _get_checkpoint_pending_writes(
        ...
        try:
            ...
            invocation_steps = self.session_client.list_invocation_steps(
                ListInvocationStepsRequest(
                    session_identifier=thread_id,
            ...
            if len(invocation_steps) == 0:
                return []

            # Get complete details for the most recent step
            invocation_step = self.session_client.get_invocation_step(
                GetInvocationStepRequest(
                    session_identifier=thread_id,
                    invocation_identifier=writes_id,
                    invocation_step_id=invocation_steps[0].invocation_step_id,
                )
            ).invocation_step
    ...
    def _get_checkpoint_step(
        ...
        return self.session_client.get_invocation_step(
            GetInvocationStepRequest(
                session_identifier=thread_id,
                invocation_identifier=invocation_id,
    ...
    def get_tuple(self, config: RunnableConfig) -> CheckpointTuple | None:
        ...
        try:
            invocation_step = self._get_checkpoint_step(
    ...
    def put_writes(
        ...
        self._create_session_invocation(thread_id, writes_invocation_identifier)

        # Get existing pending writes for this checkpoint
        current_pending_writes = self._get_checkpoint_pending_writes(
    ...
    def list(
        ...
        *,
        ...
        while True:
            try:
                response = self.session_client.list_invocation_steps(
                    ListInvocationStepsRequest(
                        session_identifier=thread_id,
                        invocation_identifier=invocation_identifier,
                        next_token=next_token,
            ...
            for step in response.invocation_step_summaries:
                ...
                step_detail = self.session_client.get_invocation_step(
                    GetInvocationStepRequest(
                        session_identifier=thread_id,
