...
class TestCreateManageMemoryTool:
    """Test suite for create_manage_memory_tool."""
    ...
    def test_create_tool_basic(self, mock_store):
        """Test creating a basic memory management tool."""
        ...
        assert "Create a memory event" in tool.description

    def test_create_tool_with_custom_params(self, mock_store):
        """Test creating tool with custom parameters."""
        custom_instructions = "Custom instructions for memory tool"
        tool = create_manage_memory_tool(
            namespace=("memories", "{user_id}"),
            instructions=custom_instructions,
            name="custom_memory_tool",
            store=mock_store,
        )

        assert tool.name == "custom_memory_tool"
        assert custom_instructions in tool.description

    def test_tool_sync_invocation(self, mock_store):
        ...
        result = tool.invoke({"content": message})

        assert "Created memory event" in result
    ...
    @pytest.mark.asyncio
    async def test_tool_async_invocation(self, mock_store):
        ...
        message = AIMessage(content="Test AI response")
        result = await tool.ainvoke({"content": message})
    ...
    def test_tool_with_runtime_namespace(self, mock_store):
        """Test tool with runtime namespace resolution."""
        tool = create_manage_memory_tool(
            namespace=("{actor_id}", "{thread_id}"),
            store=mock_store,
        )
        ...
        with patch(
            ...
            mock_get_config.return_value = {
                "configurable": {"actor_id": "user123", "thread_id": "thread456"}
            }
            ...
            assert "Created memory event" in result
            call_args = mock_store.put.call_args
            assert call_args[0][0] == ("user123", "thread456")

    def test_tool_invalid_namespace_length(self, mock_store):
        """Test tool with invalid namespace length."""
        tool = create_manage_memory_tool(
        ...
        message = HumanMessage(content="Test message")
        with pytest.raises(
            ValueError, match="namespace to be \\(actor_id, session_id\\)"
        ):
            tool.invoke({"content": message})

    def test_tool_ignores_provided_id(self, mock_store):
        """Test that tool ignores provided ID."""
        tool = create_manage_memory_tool(
            namespace=("user123", "session456"),
            store=mock_store,
        )
        ...
        message = HumanMessage(content="Test message")
        result = tool.invoke({"content": message, "id": uuid.uuid4()})

        assert "Created memory event" in result
        # Should still work, just ignore the ID

    @patch("langgraph_checkpoint_aws.agentcore.tools.get_store")
    def test_tool_without_store_uses_get_store(self, mock_get_store, mock_store):
        """Test tool without store parameter uses get_store."""
