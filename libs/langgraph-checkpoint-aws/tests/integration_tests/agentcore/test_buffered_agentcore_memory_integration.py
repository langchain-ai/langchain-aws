...
def test_agent_tools(agent_tools: list[Tool]):
    """Test the agent tools."""
    add, multiply, get_weather = agent_tools
...
class TestBufferedAgentCoreMemorySaverIntegrationSync:
    ...
    def test_flush_on_exit_with_seq_workflow(
        ...
        try:
            ...
            with buffered_agentcore_memory_saver.flush_on_exit():
                ...
                assert not buffered_agentcore_memory_saver.has_buffered_writes

                assert graph.get_state(config) is not None

            assert buffered_agentcore_memory_saver.is_empty

            persisted = agentcore_memory_saver.get_tuple(config)
            ...
            agentcore_memory_saver.delete_thread(thread_id, actor_id)
    ...
    def test_flush_mid_seq_workflow(
        ...
        try:
            ...
            assert len(list(agentcore_memory_saver.list(config))) == 2
        finally:
            agentcore_memory_saver.delete_thread(thread_id, actor_id)
            ...
            with buffered_agentcore_memory_saver.flush_on_exit():
                ...
                for event in graph.stream({"step_count": 0}, config):
                    ...
                    if node_name == "task_a":
                        assert not buffered_agentcore_memory_saver.is_empty
                        assert buffered_agentcore_memory_saver.has_buffered_checkpoint
                        assert agentcore_memory_saver.get_tuple(config) is None
                        ...
                        mid_flush_persisted = agentcore_memory_saver.get_tuple(config)
    ...
    def test_manual_flush_without_context_manager(
        ...
        try:
            ...
            flush_result = buffered_agentcore_memory_saver.flush()
    ...
    def test_multiple_sessions_isolation(
        ...
        try:
            ...
            checkpoints_1 = list(agentcore_memory_saver.list(config_1))
            checkpoints_2 = list(agentcore_memory_saver.list(config_2))
            ...
            agentcore_memory_saver.delete_thread(thread_id_1, actor_id)
            agentcore_memory_saver.delete_thread(thread_id_2, actor_id)
    ...
    def test_checkpoint_save_and_retrieve(
        ...
        try:
            ...
            persisted_tuple = agentcore_memory_saver.get_tuple(saved_config)
    ...
    def test_math_agent_with_checkpointing(
        ...
        try:
            ...
            with buffered_agentcore_memory_saver.flush_on_exit():
                ...
                checkpoint = buffered_agentcore_memory_saver.get(config)
                assert checkpoint

            persisted_checkpoint = agentcore_memory_saver.get(config)
            assert persisted_checkpoint

            checkpoint_tuples = list(agentcore_memory_saver.list(config))
            ...
            checkpoint_tuples_after = list(agentcore_memory_saver.list(config))
    ...
    def test_checkpoint_listing_with_limit(
        ...
        try:
            ...
            all_checkpoints = list(agentcore_memory_saver.list(config))
            limited_checkpoints = list(agentcore_memory_saver.list(config, limit=2))
...
class TestBufferedAgentCoreMemorySaverIntegrationAsync:
    ...
    async def test_async_flush_on_exit_with_seq_workflow(
        ...
        try:
            ...
            persisted = await agentcore_memory_saver.aget_tuple(config)
            assert persisted is not None
            assert persisted.checkpoint is not None
            assert persisted.checkpoint["channel_values"] == {
                "task_a_completed": True,
            ...
            await agentcore_memory_saver.adelete_thread(thread_id, actor_id)
            ...
            async with buffered_agentcore_memory_saver.aflush_on_exit():
                ...
                async for event in graph.astream({"step_count": 0}, config):
                    ...
                    if node_name == "task_a":
                        assert not buffered_agentcore_memory_saver.is_empty
                        assert buffered_agentcore_memory_saver.has_buffered_checkpoint
                        assert await agentcore_memory_saver.aget_tuple(config) is None
    ...
    async def test_async_manual_flush_without_context_manager(
        ...
        try:
            ...
            flush_result = await buffered_agentcore_memory_saver.aflush()
            assert flush_result is not None
            assert buffered_agentcore_memory_saver.is_empty
    ...
    async def test_async_multiple_sessions_isolation(
        ...
        try:
            ...
            await agentcore_memory_saver.adelete_thread(thread_id_1, actor_id)
            await agentcore_memory_saver.adelete_thread(thread_id_2, actor_id)
    ...
    async def test_async_checkpoint_save_and_retrieve(
        ...
        try:
            ...
            persisted_tuple = await agentcore_memory_saver.aget_tuple(saved_config)
            assert persisted_tuple is not None
            assert persisted_tuple.checkpoint["id"] == checkpoint["id"]
    ...
    async def test_async_math_agent_with_checkpointing(
        ...
        try:
            ...
            async with buffered_agentcore_memory_saver.aflush_on_exit():
                ...
                checkpoint = await buffered_agentcore_memory_saver.aget(config)
                assert checkpoint
