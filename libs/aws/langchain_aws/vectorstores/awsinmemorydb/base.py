...
from langchain_aws.vectorstores.awsinmemorydb.constants import (
...
if TYPE_CHECKING:
    ...
    from langchain_aws.vectorstores.awsinmemorydb.filters import InMemoryDBFilterExpression
    from langchain_aws.vectorstores.awsinmemorydb.schema import InMemoryDBModel
...
class AWSInMemoryDBVectorSearch(VectorStore):
    """AWSInMemoryDBVectorSearch vector database.
    ...
    .. code-block:: python
        ...
        from langchain_aws.vectorstores import AWSInMemoryDBVectorSearch
        from langchain_aws.embeddings import OpenAIEmbeddings
    ...
    Initialize, create index, and load Documents
        .. code-block:: python
            ...
            rds = AWSInMemoryDBVectorSearch.from_documents(
    ...
    Initialize, create index, and load Documents with metadata
        .. code-block:: python
            ...
            rds = AWSInMemoryDBVectorSearch.from_texts(
    ...
    Initialize, create index, and load Documents with metadata and return keys
        ...
        .. code-block:: python
            ...
            rds, keys = AWSInMemoryDBVectorSearch.from_texts_return_keys(
                texts, # a list of strings
                metadata, # a list of metadata dicts
                embeddings, # an Embeddings object
                redis_url="redis://localhost:6379",
    ...
    Initialize and connect to an existing index (from above)
        ...
        .. code-block:: python
            ...
            existing_rds = AWSInMemoryDBVectorSearch.from_existing_index(
                embeddings, # an Embeddings object
                index_name="my-index",
    ...
    schema according to the following rules:
        ...
        - All lists of strings are indexed as tag fields (joined by
            langchain_aws.vectorstores.awsinmemorydb.constants.INMEMORYDB_TAG_SEPARATOR)
    ...
    def from_texts_return_keys(
        ...
        vector_schema: Optional[Dict[str, Union[str, int]]] = None,
        **kwargs: Any,
    ) -> Tuple[AWSInMemoryDBVectorSearch, List[str]]:
        """Create a AWSInMemoryDBVectorSearch vectorstore from raw documents.

        This is a user-friendly interface that:
            1. Embeds documents.
            2. Creates a new AWSInMemoryDBVectorSearch index if it doesn't already exist
            3. Adds the documents to the newly created AWSInMemoryDBVectorSearch index.
        ...
        and print out the schema using the `AWSInMemoryDBVectorSearch.schema`` property. This
        ...
        Example:
            .. code-block:: python
                ...
                redis, keys = AWSInMemoryDBVectorSearch.from_texts_return_keys(
                    texts,
                    embeddings,
                    redis_url="redis://localhost:6379"
                )
        ...
        Returns:
            Tuple[AWSInMemoryDBVectorSearch, List[str]]: Tuple of the AWSInMemoryDBVectorSearch instance and the keys of
        ...
        try:
            ...
            from langchain_aws.vectorstores.awsinmemorydb.schema import read_schema
    ...
    @classmethod
    def from_texts(
        cls: Type[AWSInMemoryDBVectorSearch],
        texts: List[str],
        embedding: Embeddings,
        metadatas: Optional[List[dict]] = None,
        index_name: Optional[str] = None,
        ...
        **kwargs: Any,
        ...
        """Create a AWSInMemoryDBVectorSearch vectorstore from a list of texts.
        ...
        Args:
            ...
            vector_schema (Optional[Dict[str, Union[str, int]]], optional): Optional
                vector schema to use. Defaults to None.
            **kwargs (Any): Additional keyword arguments to pass to the AWSInMemoryDBVectorSearch client.

        Returns:
            AWSInMemoryDBVectorSearch: AWSInMemoryDBVectorSearch VectorStore instance.
    ...
    def from_existing_index(
        ...
        **kwargs: Any,
        ...
        """Connect to an existing AWSInMemoryDBVectorSearch index.
        ...
        Args:
            ...
            schema (Union[Dict[str, str], str, os.PathLike, Dict[str, ListOfDict]]):
                ...
                yaml file.
            key_prefix (Optional[str]): Prefix to use for all keys in AWSInMemoryDBVectorSearch associated
                with this index.
            **kwargs (Any): Additional keyword arguments to pass to the Redis client.
        ...
        # Create instance
        # init the class -- if AWSInMemoryDBVectorSearch is unavailable, will throw exception
        instance = cls(
            redis_url,
            index_name,
            embedding,
        ...
        if not check_index_exists(instance.client, index_name):
            # Will only raise if the running AWSInMemoryDBVectorSearch server does not
            # have a record of this particular index
            raise ValueError(
                f"AWSInMemoryDBVectorSearch failed to connect: Index {index_name} does not exist."
            )
    ...
    def delete(
        ...
        **kwargs: Any,
        ...
        """
        Delete a AWSInMemoryDBVectorSearch entry.
        ...
        try:
            # We need to first remove redis_url from kwargs,
            # otherwise passing it to AWSInMemoryDBVectorSearch will result in an error.
            if "redis_url" in kwargs:
                kwargs.pop("redis_url")
    ...
    def drop_index(
        ...
        **kwargs: Any,
        ...
        """
        Drop a AWSInMemoryDBVectorSearch search index.
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        # Write data to AWSInMemoryDBVectorSearch
    ...
    def as_retriever(self, **kwargs: Any) ->AWSInMemoryDBVectorStoreRetriever:
        tags = kwargs.pop("tags", None) or []
        tags.extend(self._get_retriever_tags())
        return AWSInMemoryDBVectorStoreRetriever(vectorstore=self, **kwargs, tags=tags)
    ...
    def _get_schema_with_defaults(
        ...
        from langchain_aws.vectorstores.awsinmemorydb.schema import (
            InMemoryDBModel,
            read_schema,
        )
...
class AWSInMemoryDBVectorStoreRetriever(VectorStoreRetriever):
    """Retriever for Redis VectorStore."""

    vectorstore: AWSInMemoryDBVectorSearch
    """Redis VectorStore."""
