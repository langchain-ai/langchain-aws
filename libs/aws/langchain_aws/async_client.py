...
"""Total attempts per request, matching botocore's legacy retry default."""
...
class BedrockAsyncClient:
        ...
        *,
        ...
        Args:
            ...
            config: A `botocore` `Config`. `max_pool_connections`,
                `connect_timeout`, `read_timeout`, `retries["max_attempts"]` and
                `retries["total_max_attempts"]` are honored with botocore's
                semantics. `retries["mode"]` is not: retries here always use
    ...
    def _max_attempts(self) -> int:
        ...
        Mirrors `botocore.args.ClientArgsCreator._compute_retry_max_attempts`:
        `total_max_attempts` counts the initial request and wins when both are
        set, while `max_attempts` counts only the retries after it.
        """
        retries = getattr(self._config, "retries", None) or {}
        try:
            if "total_max_attempts" in retries:
                return max(1, int(retries["total_max_attempts"]))
            retry_attempts = int(retries.get("max_attempts", _DEFAULT_MAX_ATTEMPTS - 1))
            return max(1, retry_attempts + 1)
