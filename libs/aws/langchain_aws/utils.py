...
def create_bedrock_async_client(
    ...
    Args:
        ...
        aws_secret_access_key: AWS secret access key.
        aws_session_token: AWS session token.
        endpoint_url: The complete URL to use for the constructed client. When
            omitted, the endpoint is resolved by `botocore`, so FIPS and dual-stack
            variants, partitions and the `AWS_ENDPOINT_URL[_BEDROCK_RUNTIME]`
            overrides match the synchronous client.
        config: Advanced client configuration options. `max_pool_connections`,
            `connect_timeout`, `read_timeout`, `retries["max_attempts"]` and
            `retries["total_max_attempts"]` are honored; `retries["mode"]` is not.
