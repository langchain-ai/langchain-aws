...
async def test_astream_rejects_a_stream_without_message_stop() -> None:
    """The native path enforces the same completion contract as `_stream`."""
    events: List[Dict[str, Any]] = [
        {"messageStart": {"role": "assistant"}},
    ...
    with pytest.raises(ConnectionError, match="missing messageStop"):
        [chunk async for chunk in llm.astream("hello")]


async def test_astream_rejects_an_incomplete_tool_call_stream() -> None:
    events: List[Dict[str, Any]] = [
        {"messageStart": {"role": "assistant"}},
        {
            "contentBlockStart": {
                "start": {"toolUse": {"name": "get_weather", "toolUseId": "t-1"}},
                "contentBlockIndex": 0,
            }
        },
        {
    ...
    with pytest.raises(ConnectionError, match="missing messageStop"):
        [chunk async for chunk in llm.astream("weather?")]


async def test_astream_accepts_a_complete_stream_without_metadata() -> None:
    """Metadata stays optional; only `messageStop` decides completion."""
    events: List[Dict[str, Any]] = [
        {"messageStart": {"role": "assistant"}},
    ...
    assert "".join(chunk.text for chunk in chunks) == "done"


async def test_astream_closes_the_stream_on_an_incomplete_response() -> None:
...
async def test_astream_early_consumer_close_does_not_raise() -> None:
    """A caller that stops reading has not observed an incomplete response."""
    events: List[Dict[str, Any]] = [
        {"messageStart": {"role": "assistant"}},
    ...
    chunks = cast(AsyncGenerator[Any, None], llm.astream("hello"))
    assert (await chunks.__anext__()).text is not None
    await chunks.aclose()
