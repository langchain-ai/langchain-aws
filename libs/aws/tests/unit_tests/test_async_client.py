...
async def test_retries_are_exhausted_then_raise() -> None:
    ...
    client, http_client = _client(
        throttled, config=mock.MagicMock(retries={"max_attempts": 1})
    )
...
def _service_model_header_members(operation: str) -> Dict[str, str]:
    """Read the header-bound input members botocore models for an operation."""
    from botocore.session import get_session

    model = get_session().get_service_model("bedrock-runtime")
    input_shape = model.operation_model(operation).input_shape
    assert input_shape is not None
    members = input_shape.members
    return {
        name: member.serialization["name"]
        for name, member in members.items()
        if member.serialization.get("location") == "header"
    }


def test_invoke_header_members_match_the_service_model() -> None:
    """Catch drift between the transcribed header maps and botocore's model."""
    assert _service_model_header_members("InvokeModel") == dict(_INVOKE_HEADER_MEMBERS)
    assert _service_model_header_members("InvokeModelWithResponseStream") == dict(
        _INVOKE_STREAM_HEADER_MEMBERS
    )


def test_converse_operations_have_no_header_members() -> None:
    """Converse kwargs are all body-bound, as `converse()` assumes."""
    assert _service_model_header_members("Converse") == {}
    assert _service_model_header_members("ConverseStream") == {}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"max_attempts": 2}, 3),
        ({"max_attempts": 4}, 5),
        ({"total_max_attempts": 3}, 3),
        ({"total_max_attempts": 3, "max_attempts": 9}, 3),
        ({}, 5),
    ],
)
def test_max_attempts_follows_botocore_semantics(
    retries: Dict[str, int], expected: int
) -> None:
    """`max_attempts` counts retries; `total_max_attempts` counts every send."""
    client, _ = _client(config=mock.MagicMock(retries=retries))

    assert client._max_attempts == expected
