...
def test_streaming_with_nova_grounding() -> None:
...
def test_streaming_with_nova_code_interpreter() -> None:
...
def test_streaming_reasoning_content_chunks() -> None:
    ...
    assert len(reasoning_blocks) > 0, "Expected reasoning content blocks in streamed message"


def test_streaming_tool_use_and_result_events() -> None:
    ...
    for chunk in model_with_tools.stream("Use Python to calculate the square root of 144"):
        chunks.append(chunk)
...
def test_streaming_accumulating_chunks_into_full_message() -> None:
...
def test_streaming_with_both_system_tools() -> None:
...
def test_streaming_reasoning_with_different_effort_levels() -> None:
    ...
    for effort in ["low", "medium", "high"]:
        ...
        chunks = []
        for chunk in model_with_tools.stream(f"Calculate 15 factorial"):
            chunks.append(chunk)
...
def test_streaming_chunk_content_structure() -> None:
...
def test_streaming_with_string_tool_names() -> None:
    ...
    model_with_tools = model.bind_tools(["nova_grounding", "nova_code_interpreter"])
...
def test_streaming_empty_chunks_handling() -> None:
